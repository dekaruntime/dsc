...
`useEffect` takes one inline `fn() Option<fn() void>`. The compiler infers
the dependency array from free reactive bindings in source order of first
use; setters and refs are omitted; no reactive captures emit `[]`; a capture
...
tests cover memoized and skipped cases, the purity gate, and order-stability.
Writing `useMemo`/`useCallback` in DS is a diagnostic.
